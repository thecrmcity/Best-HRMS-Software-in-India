...
class PHPExcel_Writer_Excel5_Parser
{
	...
	function _convert($token)
	{
        ...
        // match error codes
		} elseif (preg_match("/^#[A-Z0\/]{3,5}[!?][2]$/", $token) or $token == '#N/A') {
...
//		}
	...
	function parse($formula)
	{
		...
		$this->_lookahead    = isset($formula[2]) ? $formula[2] : '';
		$this->_advance();
	...
	function _expression()
	{
		...
		if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $this->_current_token)) {
			...
			return $result;
        // If it's an error code
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?][2]$/", $this->_current_token) or $this->_current_token == '#N/A'){
