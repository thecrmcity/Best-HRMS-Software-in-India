    ...
    switch($case)
    {
        ...
        <div class="timeline">
            <?php
              ...
              $cond = " `in_comid`='$comid' AND `in_empid`='$id' AND `in_status`='1' ";
              $select = new Selectall();
              $getdox = $select->getallCond($table,$cond);
              if(!empty($getdox))
              {
                foreach($getdox as $dox)
                {
                    $docid = $dox['in_docid'];
                    $select = new Selectall();
                    ?>
                    <div class="time-label">
                        <span class="bg-red"><?php echo date('d F,Y', strtotime($dox['in_sendtime']))?></span>
                    </div>
                    <div>
                        <i class="far fa-file-pdf bg-danger"></i>
                        <div class="timeline-item">
                        <!-- <span class="time"><i class="fas fa-clock"></i> 5 mins ago</span> -->
                        <h3 class="timeline-header no-border"><?php echo $select->getTemplate($docid);?></h3>
                        </div>
                    </div>
                    <?php
                }
